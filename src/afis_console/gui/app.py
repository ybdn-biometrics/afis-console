...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.folder_entry = ctk.CTkEntry(self.selection_frame, textvariable=self.folder_path, placeholder_text="Aucun dossier sélectionné", state="readonly")
        self.folder_entry.grid(row=0, column=0, padx=10, pady=10, sticky="ew")

        self.browse_button = ctk.CTkButton(self.selection_frame, text="Sélectionner Dossier", command=self.select_folder)
        ...
        self.action_button.grid(row=1, column=0, columnspan=2, padx=10, pady=(0, 10), sticky="ew")
    ...
    def start_process(self):
        ...
        thread = threading.Thread(target=self.run_logic, args=(source_dir,))
        thread.start()

    def run_logic(self, source_dir):
        try:
            if logic:
                def safe_log(msg):
                    self.after(0, lambda: self.log_message(msg))

                stats = logic.process_folder(source_dir, log_callback=safe_log)
